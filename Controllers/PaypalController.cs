...
public class PaypalController : Controller
{
    ...
    public PaypalController( IComprasRepository ComprasRepository, IOrdenCompraRepository OrdenCompraRepository, IProductosRepository repository, IMapper mapper)
    {
    ...
    [HttpPost("Paypal/Compra/{compraId}")]
    [Authorize]
    ...
    public async Task<ActionResult> Paypal(int compraId)

        {
            ...
            {
                if (usuarioActual.Id != compra.UsuarioId)
                    return NotFound("No tenes los permisos para pagar esta Compra");
            }
           ...
           new TpiBarberShop.Models.Paypal.PurchaseUnit()
           {
		    ...
		    //"description":"Pruducto de pruebas"
                    ...
                    application_context = new ApplicationContext()
                    {
                        ...
                        return_url = $"http://localhost:3000/VerificarToken?compraId={compraId}",
    ...
    [HttpPost("Transaction/OrdenCompras")]
    public async Task<ActionResult> TransactionOrden(string token, int ordenCompraId)
    {
        bool status = false;
        var compra = _OrdenCompraRepository.GetOrdenCompra(ordenCompraId);
    ...
    [HttpPost("Paypal/OrdenCompra/{ordenCompraId}")]
    [Authorize]
    //public async Task<JsonResult> Paypal(PaypalDTO paypaldto)
    public async Task<ActionResult> PaypalOrden(int ordenCompraId)

    {

        var compras = _OrdenCompraRepository.GetOrdenCompra(ordenCompraId);
        var compra = _mapper.Map<OrdenCompraDTO>(compras);
        if (compra == null)
        {
            return NotFound("No existe esa Compra");
        }
           ...
           new TpiBarberShop.Models.Paypal.PurchaseUnit()
           {
		    ...
		    //"description":"Pruducto de pruebas"
                ...
                application_context = new ApplicationContext()
                {
                    brand_name = "NavajaFina.com",
                    landing_page = "NO_PREFERENCE",
                    return_url = $"http://localhost:3000/VerificarToken?ordenCompraId={ordenCompraId}",
                    cancel_url = "http://localhost:3000"
